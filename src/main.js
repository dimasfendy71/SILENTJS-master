...
const myAdmin = ['uf6d3d5fa37246cb2fe3b4a949729129e','uebe39828703a6951128ae52b85c29302','ud74e38c4062825cb14799700cfd4e2e3']; //Mid Lu Supaya Jadi Admin

const myAssist = ['u04a2bc47eb96422f381a53cf7909f2ce','u135f1dd7a9edd56248df673e1a4dfdef','u23abf64b9e0f7b73c11d708dc4597be6']; //Mid Bot Lu Jika Ada Bot 2 Atau Lebih

const myBot = ['uee4e5baa550f96897102066006506ee9','u60ea022b974a1d2aab755dfaae9d8e04','ua2a9cf7ad7df850374f06f1955db4af2','u1af11dfe022c30161c18e022321ca3bd','u7cf68aadd183e2978973621cc9826423']; //Taro Mid Bot Lu Kesini (Maksud Gw Bot Yang Pake Sc Ini)
const BoT = ['uee4e5baa550f96897102066006506ee9','u60ea022b974a1d2aab755dfaae9d8e04','ua2a9cf7ad7df850374f06f1955db4af2','u1af11dfe022c30161c18e022321ca3bd','u7cf68aadd183e2978973621cc9826423']
var BlackListPermanen = ['uf6d3d5fa37246cb2fe3b4a949729129e'];
var BackupList = [];
var TbanList = []; //Users Banned Chat
var commandGroup = [];
var banList = []; //Banned list
var vx = {};var midnornama,pesane,kickhim;var waitMsg = "no";//DO NOT CHANGE THIS
var komenTL = "AutoLike by \nline://ti/p/~oooo"; //Comment for timeline
      ...
      if(txt == "msg" && isAdmin(seq.from_)) {
       ...
       if(txt == "del:staff"){
        ...
        if(txt == 'response name') {
           if(isAdmin(seq.from_) || isStaff(seq.from_)) {
            this._sendMessage(seq, 'aku datanggg🏃‍♂️🏃‍♂️🏃‍♂️ ');
           }
        }
...
await this._sendMessage(seq,mentions.names.join(''))
        ...
        if(txt === 'yes' && this.sendRata == 1 && isAdmin(seq.from_)) {
            this.sendRata = 0;
            let txt = await this._sendMessage(seq, ' ☄☄WELCOME ☄☄');
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
                if(isAdmin(listMember[i].mid)) {
                }
       ...
       if(txt == "kickall" || txt == "ban all users"){
        ...
        if(txt == 'creatorbot') {
           this._sendMessage(seq, 'My Creator Is Bee\nId Line : http://line.me/ti/p/~dimas.fendy\n\n-dimas вσт-');
           seq.contentType=13;ufdc20b3a00b5e8f31e4f91017eb361b0
           seq.contentMetadata = { mid: 'uf6d3d5fa37246cb2fe3b4a949729129e' };
...
        }
        ...
        if(cmd == 'spam' && isStaff(seq.from_)) {
            for(var i= 0; i < 10;  i++) {
               this._sendMessage(seq, '☠ⅅⅈℳᗅՏ ℬᝪᝨՏ™☠');
        }
    }
        ...
        if(txt == 'bye') {
           if(isAdmin(seq.from_) || isStaff(seq.from_)){
          let txt = await this._sendMessage(seq, 'Terima Kami Izin Leave ');
